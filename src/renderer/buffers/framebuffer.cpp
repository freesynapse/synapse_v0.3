...
//-----------------------------------------------------------------------------------
void framebuffer_base_t::bind(bool _set_viewport, bool _restore_prev_framebuffer)
{
	m_restorePrevFramebuffer = _restore_prev_framebuffer;
	// if (m_restorePrevFramebuffer)
    		// m_prevFramebufferID = renderer.get_current_framebuffer();
	// renderer.set_current_framebuffer(m_framebufferID);
...
{
	if (!m_restorePrevFramebuffer) {
        m_restorePrevFramebuffer = false;
...
//-----------------------------------------------------------------------------------
void framebuffer_base_t::saveAsPNG(const std::string &_file_path/* ="" */)
{
...
void framebuffer_base_t::resize(const glm::ivec2 &_size)
{
	...
	if (m_framebufferID) {
		glDeleteFramebuffers(1, &m_framebufferID);
		glDeleteTextures(m_colorAttachmentCount, m_colorAttachmentID);
	...
	glGenTextures(m_colorAttachmentCount, m_colorAttachmentID);
	GLenum drawBufferIDs[m_colorAttachmentCount];
	...
	for (size_t i = 0; i < m_colorAttachmentCount; i++) {
		...
		glTexImage2D(GL_TEXTURE_2D, 
			         0, 
			         m_pxFmt.internalFormat, 
			         m_size.x, 
			         m_size.y, 
			         0, 
			         m_pxFmt.storageFormat, 
			         m_pxFmt.storageType, 
			         NULL);
	...
	glDrawBuffers(m_colorAttachmentCount, drawBufferIDs);
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
		...
		SYN_TRACE("Framebuffer '%s' (renderTargets=%d [%d x %d x %d]) created.\n", 
		          m_name.c_str(), m_colorAttachmentCount, m_size.x, m_size.y, getPixelFmtChannels(m_format));
	#endif
...
//-----------------------------------------------------------------------------------
void framebuffer_base_t::bindTexture(uint32_t _tex_slot, GLuint _color_attachment_slot) const
{
	glActiveTexture(GL_TEXTURE0 + _tex_slot);
	glBindTexture(GL_TEXTURE_2D, m_colorAttachmentID[_color_attachment_slot]);
	//glBindTextureUnit(_tex_slot, m_colorAttachmentID[_color_attachment_slot]);
}

//-----------------------------------------------------------------------------------
void framebuffer_base_t::bindTexture(uint32_t _tex_slot, 
