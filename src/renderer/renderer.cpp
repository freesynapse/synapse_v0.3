...
static void __renderer_on_resize_callback(const event_t &_e)
{
    renderer.on_resize(_e);
...
void renderer_t::init_material_ubo() {
  ...
  glNamedBufferData(m_material_ubo.opengl_id, m_material_ubo.size, nullptr,
                    GL_DYNAMIC_DRAW);

  // bind the hardware asset space directly to the engine's global target index
  // slot
  glBindBufferBase(GL_UNIFORM_BUFFER, m_material_ubo.binding_point,
                   m_material_ubo.opengl_id);
}
...
void renderer_t::init_lighting_ubo() {
  ...
  // allocate immutable or dynamic device memory block spaces directly
  glNamedBufferData(m_lighting_ubo.opengl_id, m_lighting_ubo.size, nullptr,
                    GL_DYNAMIC_DRAW);

  // bind the hardware asset space directly to the engine's global target index
  // slot
  glBindBufferBase(GL_UNIFORM_BUFFER, m_lighting_ubo.binding_point,
                   m_lighting_ubo.opengl_id);
...
void renderer_t::update_lighting_ubo() {
  glNamedBufferSubData(m_lighting_ubo.opengl_id, 0, m_lighting_ubo.size,
                       &m_lighting_state);
}
...
void renderer_t::init_skybox() {
  ...
  shader_handle_t shader_handle = shader_lib.load_from_file(
      "skybox_shader", "../assets/shaders/skybox.glsl");

  if (!mesh_handle.is_valid() || !shader_handle.is_valid()) {
    SYN_WARNING(
        "skybox initialization failed. mesh handle %d, shader handle %d.\n",
...
void renderer_t::render_skybox() {
  ...
  sky_shader->set_matrix_4fv("u_projection",
                             orbit_camera.get_projection_matrix());

  cubemap_internal_t *cubemap =
      cubemap_lib.get_cubemap(m_skybox.cubemap_handle);
...
void renderer_t::bake_irradiance_hdr() {
  ...
  // single use
  shader_t *conv_shader = shader_lib.get_shader(shader_lib.load_from_file(
      "irradiance_shader", "../assets/shaders/ibl_irradiance.glsl"));
  conv_shader->enable();
  conv_shader->set_matrix_4fv("u_projection", capture_proj);

  cubemap_internal_t *env_map =
      cubemap_lib.get_cubemap(m_skybox.cubemap_handle);
  glActiveTexture(GL_TEXTURE0);
...
{ 
    ...
    shader_t *spec_shader = shader_lib.get_shader(shader_lib.load_from_file(
        "prefilter_conv", "../assets/shaders/ibl_prefilter.glsl"));
    spec_shader->enable();
...
//
framebuffer_handle_t renderer_t::create_framebuffer(
    const color_format_t &_format, const glm::ivec2 &_size,
    size_t _n_drawbuffers, bool _use_depthbuffer, const std::string &_name) {
  ...
  m_framebuffers[slot].create(_format, _size, _n_drawbuffers, _use_depthbuffer,
                              _name);
  m_frambuffer_count++;
  return {slot + 1};
}
...
//
framebuffer_t *
renderer_t::get_framebuffer(const framebuffer_handle_t &_handle) {
...
void renderer_t::create_scene_framebuffer() {
  ...
  // m_scene_fbuffer_sptr->create(color_format_t::RGBA16F, glm::ivec2(0), 1,
  // true, "scene_fbuffer");

  // SYN_INFO("created framebuffer '%s' (%dx%d).\n",
  // m_scene_fbuffer_sptr->getName().c_str(), m_scene_fbuffer_sptr->getWidth(),
  // m_scene_fbuffer_sptr->getHeight());

  m_scene_fbuffer_handle = create_framebuffer(
      color_format_t::RGBA16F, glm::ivec2(0), 1, true, "scene_fbuffer");
  ...
  // create the shader
  m_scene_fbuffer_shader_handle = shader_lib.load_from_file(
      "scene_buffer_shader", "../assets/shaders/scene_fbuffer.glsl");
...
void renderer_t::cmd_flush() {
  ...
  for (uint32_t i = 0; i < m_command_count; i++) {
    ...
    if (i == 0 || m_command_queue[i - 1].material.id != cmd.material.id) {
      ...
      for (uint32_t slot = 0; slot < (uint32_t)texture_map_type_t::COUNT;
           slot++) {
    ...
    // bind irradiance map
    cubemap_internal_t *irradiance_map =
        cubemap_lib.get_cubemap(m_irradiance_map);
    if (irradiance_map) {
    ...
    // bind prefilter map
    cubemap_internal_t *prefilter_map =
        cubemap_lib.get_cubemap(m_prefilter_map);
    if (prefilter_map) {
...
void renderer_t::record_frame_time(float _dt_ms) {
  ...
  m_perf_stats.frame_time_idx =
      (m_perf_stats.frame_time_idx + 1) % SYN_PERF_GRAPH_SAMPLE_COUNT;

  // update max for auto-scaling
  m_perf_stats.max_frame_time = 0.0f;
  for (uint32_t i = 0; i < SYN_PERF_GRAPH_SAMPLE_COUNT; i++) {
    m_perf_stats.max_frame_time =
        std::max(m_perf_stats.max_frame_time, m_perf_stats.frame_times[i]);
  }
...
void renderer_t::init_perf_graph() {
  ...
  m_perf_stats.graph_vao.create_empty_vertices(SYN_PERF_GRAPH_SAMPLE_COUNT * 4 *
                                               6 * sizeof(float));
  m_perf_stats.graph_vao.create_empty_indices(SYN_PERF_GRAPH_SAMPLE_COUNT * 6 *
                                              sizeof(uint32_t));

  m_perf_stats.graph_shader_handle = shader_lib.load_from_file(
      "ui_perf_stats", "../assets/shaders/ui_perf_stats.glsl");
...
void renderer_t::draw_perf_stats() {
  ...
  // text
  float text_y = y;
  font.render_text(x, text_y += line_height, "FPS: %d (%.2f ms)", time_step.fps,
                   time_step.dt * 1000.0f);
  font.render_text(x, text_y += line_height, "Draw Calls: %d",
                   m_perf_stats.draw_calls_per_frame);
  ...
  if (m_notification.display_time > 0.0f) {
    float alpha = glm::clamp(
        m_notification.display_time / m_notification.duration, 0.0f, 1.0f);
...
void renderer_t::draw_frame_time_graph(float _x, float _y, float _w, float _h) {
  ...
  float max_frame_time =
      (m_perf_stats.max_frame_time < 20.0f ? 20.0f
                                           : m_perf_stats.max_frame_time);

  for (uint32_t i = 0; i < SYN_PERF_GRAPH_SAMPLE_COUNT; i++) {
    uint32_t sample_idx =
        (m_perf_stats.frame_time_idx + i) % SYN_PERF_GRAPH_SAMPLE_COUNT;
...
void renderer_t::create_orienatation_obj(uint32_t _size) {
  ...
  // create shader
  m_orientation_obj_shader_handle = shader_lib.load_from_file(
      "orientation_obj_shader", "../assets/shaders/debug/orientation_obj.glsl");
  ...
  orientation_obj_v vertices[6] = {// x
  ...
  m_orientation_obj_vao.create(
      vertices, sizeof(vertices) / sizeof(orientation_obj_v), nullptr, 0);
...
void renderer_t::render_orientation_obj() {
  ...
  static glm::mat4 ortho_proj =
      glm::ortho(-1.1f, 1.1f, -1.1f, 1.1f, -1.1f, 1.1f);
...
void renderer_t::init_debug_rendering() {
  m_debug.normal_shader_handle = shader_lib.load_from_file(
      "debug_normal_shader", "../assets/shaders/debug/debug_mesh_normals.glsl");

  m_debug.line_shader_handle = shader_lib.load_from_file(
      "debug_line_shader", "../assets/shaders/debug/debug_lines.glsl");
  ...
  // grid
  m_debug.grid_shader_handle = shader_lib.load_from_file(
      "debug_grid_shader", "../assets/shaders/debug/debug_grid.glsl");
...
//
void renderer_t::toggle_wireframe() {
  m_debug.show_wireframe = !m_debug.show_wireframe;
}
void renderer_t::toggle_normals() {
  m_debug.show_normals = !m_debug.show_normals;
}
void renderer_t::toggle_tangents() {
  m_debug.show_tangents = !m_debug.show_tangents;
}
void renderer_t::toggle_bounding_boxes() {
  m_debug.show_bounding_boxes = !m_debug.show_bounding_boxes;
}
void renderer_t::toggle_grid() { m_debug.show_grid = !m_debug.show_grid; }

//
void renderer_t::draw_debug_normals(mesh_handle_t _mesh_handle,
                                    const glm::mat4 &_transform) {
  if (!m_debug.show_normals && !m_debug.show_tangents)
    return;
  if (!m_debug_initialized)
    return;
...
void renderer_t::draw_debug_bounding_box_entities(entity_t *_entity) {
  if (!_entity) {
    for (uint32_t i = 0; i < entity_lib.m_active_count; i++) {
      ...
      if (mesh) {
        renderer.draw_debug_bounding_boxes(mesh->aabb_min, mesh->aabb_max,
                                           e->transform);
      }
    }
  } else {
...
void renderer_t::draw_debug_bounding_boxes(const glm::vec3 &_min,
                                           const glm::vec3 &_max,
                                           const glm::mat4 &_transform) {
  if (!m_debug.show_bounding_boxes)
    return;
  if (!m_debug_initialized)
    init_debug_rendering();
  ...
  auto add_line = [&](int a, int b) {
    // vertex 0
    lines.push_back(corners[a].x);
    lines.push_back(corners[a].y);
    lines.push_back(corners[a].z);
    lines.push_back(color.r);
    lines.push_back(color.g);
    lines.push_back(color.b);
    lines.push_back(color.a);
    // vertex 1
    lines.push_back(corners[b].x);
    lines.push_back(corners[b].y);
    lines.push_back(corners[b].z);
    lines.push_back(color.r);
    lines.push_back(color.g);
    lines.push_back(color.b);
    lines.push_back(color.a);
  ...
  m_debug.line_vao.update_vertices((void *)&lines[0],
  ...
  shader->enable();
  shader->set_matrix_4fv("u_view_projection",
                         orbit_camera.get_view_projection_matrix());
